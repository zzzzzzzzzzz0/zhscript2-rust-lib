...
void save_image(torch::Tensor tensor, const std::string& file_path, int64_t nrow, int64_t padding,
    ...
    switch (format) {
        case ImageFormat::PNG: //printf("%lu %lu\n",grid.size(2),grid.stride(0));
...
torch::Tensor load_image(const std::string& file_path,
    torch::IntArrayRef shape, std::function<torch::Tensor(torch::Tensor)> transform, int* ret_width, int* ret_height) {
    ...
    if(ret_width) *ret_width = new_width;
    if(ret_height) *ret_height = new_height;
