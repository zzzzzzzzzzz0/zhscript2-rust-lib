...
int main()
{
	...
	auto z__ = [&](cv::Mat& frame, clock_t start) {
		cv::Mat img;
		...
		if (dets.size() > 0)
		{
			std::cout << "auto frame_cols=" << frame.cols << ", frame_rows=" << frame.rows << "; " << std::endl;
			float w_s = frame.cols / 640., h_s = frame.rows / 384.;
			std::cout << w_s << " " << h_s << " " << std::endl;
		    // Visualize result
		    for (size_t i=0; i < dets[0].sizes()[0]; ++ i)
		    {
		        float left = dets[0][i][0].item().toFloat() * w_s;
		        float top = dets[0][i][1].item().toFloat() * h_s;
		        float right = dets[0][i][2].item().toFloat() * w_s;
		        float bottom = dets[0][i][3].item().toFloat() * h_s;
		        float score = dets[0][i][4].item().toFloat();
		        int classID = dets[0][i][5].item().toInt();

				std::cout << i << ") " << left << "," << top << "," << right << "," << bottom << " " << score << " " << classID << "/" << classnames[classID] << std::endl;

				cv::rectangle(frame, cv::Rect(left, top, (right - left), (bottom - top)), cv::Scalar(0, 255, 0), 2);

				auto s = classnames[classID] + ": " + cv::format("%.2f", score);
				std::cout << s << std::endl;
				cv::putText(frame,
					s,
					cv::Point(left, top),
		...
		auto s = "FPS: " + std::to_string(int(1e7 / (clock() - start)));
		std::cout << s << std::endl;
		cv::putText(frame, s,
		    cv::Point(50, 50),
